...
volatile bool ft6336_touch_flag = false;
volatile bool idle_wake_flag=false;

 void IRAM_ATTR ft6336_interrupt_handler() {

     ft6336_touch_flag = true;
	 idle_wake_flag=true;
 }
...
pinMode(FT_RST_pin, OUTPUT); //RST 
pinMode(FT_SCL_Pin, OUTPUT); //SCL 
pinMode(FT_SDA_Pin, OUTPUT); //SDA
...
//	i2c_write_addr_byte(0x38,FT_DEVIDE_MODE,temp);
	...
	FT6336_RD_Reg(FT_ID_G_PERIODACTIVE,&KK,1);
     pinMode(IRQ_Pin, INPUT_PULLUP);
     attachInterrupt(digitalPinToInterrupt(IRQ_Pin), ft6336_interrupt_handler, FALLING);
